...
describe("renderMarkdown", () => {
  ...
  it("includes footer with typehunt link", () => {
    ...
    expect(md).toContain("github.com/CihanBir7/typehunt");
  });
...
describe("renderTextReport", () => {
  it("renders summary and duplicate sections", () => {
    const d1 = makeDeclaration({ file: "a.ts", line: 10, name: "Foo" });
    const d2 = makeDeclaration({ file: "b.ts", line: 20, name: "Foo" });
    const d3 = makeDeclaration({ file: "c.ts", line: 30, name: "Bar" });
    ...
    expect(text).toContain("Duplicate name groups:");
    expect(text).toContain("Duplicate shape groups:");
    expect(text).toContain("── Duplicate type names");
    expect(text).toContain("── Duplicate type shapes");
  });

  it("renders no-duplicate lines for empty groups", () => {
    const text = renderTextReport([], [], makeMeta({ mode: "both" }));
